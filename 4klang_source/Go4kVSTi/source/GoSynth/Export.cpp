...
Recorder::Recorder(const SynthObject &synthObj, bool recordingNoise, float bpm, int patternsize, float patternquant)
	: SynthObj(synthObj)
	, RecordingNoise(recordingNoise)
	, TickScaler(patternquant)
	, PatternSize(patternsize * patternquant)
	, BeatsPerMinute(bpm)
	, SamplesPerTick((TickScaler >= 1.0f)
		? 44100.0f*4.0f*60.0f/(bpm*16*patternquant)
		: 44100.0f*4.0f*60.0f/(bpm*16))
{
	// clear all buffers
	for (int i = 0; i < MAX_INSTRUMENTS; i++)
	{
		InstrumentOn[i] = -1;
		InstrumentRecord[i].resize(256 * 256, 0);
	}
}

void Recorder::tick(int samples)
{
	samplesProcessed += samples;
}

// add a voice with given parameters to synth
void Recorder::voiceAdd(int channel, int note)
{
	// record song
...
void Recorder::voiceStop(int channel, int note)
{
	int CurrentTick = (int)(0.5f + ((float)samplesProcessed / SamplesPerTick));
	if (InstrumentOn[channel] >= 0)
	{
...
void Recorder::finishAndSaveByteStream(std::unique_ptr<Recorder>&& recorder, const char* filename, int useenvlevels, int useenotevalues, int clipoutput, int undenormalize, int objformat, int output16)
{
...
static void GetUses(SynthUses *uses, bool InstrumentUsed[], const SynthObject &SynthObj)
{
	for (int i = 0; i <= MAX_INSTRUMENTS; i++)
	{
...
void Recorder::write(const char* filename, int useenvlevels, int useenotevalues, int clipoutput, int undenormalize, int objformat, int output16)
{
...
#endif
		GetUses(&uses, InstrumentUsed, SynthObj);
